use crate::{
    bitboard::{clear_bit, get_bit, rf_to_square_i8},
    constants::BISHOP_MAGIC_NUMBERS,
    enums::{Color, PieceAndColor},
    position::Position,
};

use super::{mask_occupancy, Move};

#[derive(Clone)]
pub struct BishopMoveGen {
    candidate_table: [u64; 64],
    relevant_bits_table: [u8; 64],
    attack_table: Vec<Vec<u64>>, // originally [[u64; 512]; 64] but this exceeds the stack size
}
...
impl BishopMoveGen {
    ...
    pub fn generate_candidate_mask(square: u8) -> u64 {
        ...
        while rank < 7 && file < 7 {
            attacks |= 1 << rf_to_square_i8(rank, file);
    ...
    pub fn generate_attack_mask(square: u8, occupancy: u64) -> u64 {
        ...
        while rank <= 7 && file <= 7 {
            let mask = 1 << rf_to_square_i8(rank, file);
    ...
    fn generate_attack_table(
        ...
        let mut attack_table = vec![vec![0u64; 512]; 64];
    ...
    pub fn generate_moves(&self, position: &Position) -> Vec<Move> {
        ...
        let piece = match color {
            Color::White => PieceAndColor::WhiteBishop,
            Color::Black => PieceAndColor::BlackBishop,
            _ => unreachable!(),
        };
